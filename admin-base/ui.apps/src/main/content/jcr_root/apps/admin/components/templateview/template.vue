...
<script>
  ...
  methods: {
    ...
    renameTemplate() {
      ...
      if(newName) {
        $perAdminApp.stateAction('renameTemplate', { path: this.template.path, name: newName})
        let newPath = this.currentObject.split('/')
        newPath.pop()
        newPath.push(newName)
        $perAdminApp.stateAction('showTemplateInfo', { selected: newPath.join('/') })
      }
    },
    deleteTemplate() {
      $perAdminApp.stateAction('deleteTemplate', this.template.path)
    ...
    onOk() {
      $perAdminApp.stateAction('saveTemplateProperties', this.template )
