    ...
    <div class="explorer-layout">
      <div class="row">
          ...
          <ul class="collection">
            ...
            <li
              ...
              <admin-components-action v-if="editable(child)"
                        ...
                        }"><i class="material-icons">{{nodeTypeToIcon(child.resourceType)}}</i>
              ...
              <admin-components-extensions
                  v-bind:model="{id: 'admin.components.explorer', item: child}"></admin-components-extensions>
        ...
        <admin-components-explorerpreview v-if="hasEdit">
          <component v-bind:is="model.children[0].component"
                     v-bind:model="model.children[0]"></component>
      ...
      <div class="file-upload-inner">
        ...
        <div class="file-upload-action">
          <button
              type="button"
              class="btn"
              v-on:click="onDoneFileUpload">ok
          </button>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      nodeTypeToIcon: function (nodeType) {
        ...
        if (nodeType === 'per:Page') {
          return 'description'
        }
        if (nodeType === 'per:Object') {
          return 'layers'
        }
        if (nodeType === 'nt:file') {
          return 'insert_drive_file'
        }
        if (nodeType === 'per:Asset') {
          return 'image'
        }
        if (nodeType === 'sling:Folder') {
          return 'folder'
        }
        if (nodeType === 'sling:OrderedFolder') {
          return 'folder'
        }
        return 'unknown'
      },
      checkIfAllowed: function (resourceType) {
        return ['per:Asset', 'nt:file', 'sling:Folder', 'sling:OrderedFolder', 'per:Page',
          'sling:OrderedFolder', 'per:Object'].indexOf(resourceType) >= 0
      },
...
//                    }
        ...
        $perAdminApp.stateAction('selectToolsNodesPath',
            {selected: target.path, path: me.model.dataFrom})
      },
...
//                    $perAdminApp.notifyUser('create new site', 'to create a new site root, please visit the documentation on how to start a new site', null)
      ...
      addTemplate: function (me, target) {
        if (me.pt.path === '/content/templates') {
          $perAdminApp.notifyUser('create new site',
              'to create a new site root, please visit the documentation on how to start a new site',
              null)
        } else {
      ...
      deletePage: function (me, target) {
        $perAdminApp.askUser('Delete Page',
            me.$i18n('Are you sure you want to delete this node and all its children?'), {
              yes() {
      ...
      deleteSite: function (me, target) {
        $perAdminApp.askUser('Delete Site', me.$i18n(
