...
public class ComponentDefinitionServlet extends AbstractBaseServlet {
  ...
  protected Response handleRequest(Request request) throws IOException {
    ...
    if (resource == null) {
      ...
      return new ErrorResponse().setErrorCode(404)
          .setErrorMessage("Resource with Path: '" + path + "' not found");
    }
    boolean page = false;
    if (resource.getResourceType().equals(PerConstants.PAGE_PRIMARY_TYPE)) {
      ...
      if (jcrContent == null) {
        return new ErrorResponse().setErrorCode(404)
            .setErrorMessage("Page with Path: '" + path + "' has no Content Chile Node");
      }
      page = true;
    }
    ...
    if (path.startsWith(APPS_ROOT + SLASH)) {
      ...
      componentPath =
          APPS_ROOT + SLASH + resource.getValueMap().get(SLING_RESOURCE_TYPE, String.class);
    }
    ...
    if (component == null) {
      // In case the resource could not be found issue an Error Resposne
      return new ErrorResponse().setErrorCode(404)
          .setErrorMessage("Component Resource with Path: '" + componentPath + "' not found");
    }
    ...
    if (dialog != null) {
      answer.writeAttributeRaw(MODEL,
          ServletHelper.asString(dialog.adaptTo(InputStream.class)).toString());
    }
    if (ogTags != null) {
      answer.writeAttributeRaw(OG_TAGS,
          ServletHelper.asString(ogTags.adaptTo(InputStream.class)).toString());
    }
    return answer;
  }

  private @NotNull Resource getDialogFromSuperType(@NotNull Resource resource, boolean page, boolean og) {
